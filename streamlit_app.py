...
if api_key:
    ...
    def create_analyze_content_task(review_text_present):
        if review_text_present:
            ...
            "Strictly ensure that if the review text contains 2 or fewer sentences, where a  sentence is defined as a portion of text ending with a single full stop (.), and **not a comma (,)**, and the provided rating is 1, 2, 3, or 4 the output should be 'Justified'."

            "Strictly make sure the second priority analyis is done when the Review text is more than two sentences only"
